...
class POOL {
 ...
 public:
	...
	POOL(void);
	POOL(DByte *p, DInt l);
	void init_pool(DByte *p, DInt l);
	DByte *put(DByte *p, DInt l);
	DByte *next(void);
	void release(void);
	DUint alloc(DInt l);
...
class ServerTSMTable {
 ...
 public:
  ServerTSMTable(void);
  DInt lookid(DInt n, DInt d, DInt s, DByte service, DInt id, POOL *spool);
...
DInt ServerTSMTable::lookid(DInt n, DInt d, DInt s, DByte service, DInt id, POOL *spool)
{
 DInt i,j;
 j=-1;
