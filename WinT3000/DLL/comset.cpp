...
extern "C" __declspec(dllexport) HANDLE InitCommPort(COMMINFO *pCom);
extern "C" __declspec(dllexport) bool WriteString(HANDLE h, LPSTR lpData, DWORD dwLength);
...
HANDLE InitCommPort(COMMINFO *pCom)
{
 ...
 hCom = CreateFile(
    strcat("COM",itoa(pCom->PortName+1,buf,10)),
